        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl p-8 animate-fade-in max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              <div className="bg-gray-50 p-6 rounded-xl border border-gray-100"><h3 className="font-bold text-gray-700 mb-3 uppercase text-xs flex items-center gap-2"><FaTruck /> Shipping Information</h3>{selectedOrder.shippingAddress ? (<div className="space-y-1 text-sm text-gray-600"><p className="font-bold text-gray-900">{selectedOrder.shippingAddress.fullName || selectedOrder.user?.name || "Guest"}</p><p>{selectedOrder.shippingAddress.address}</p><p>{selectedOrder.shippingAddress.city}, {selectedOrder.shippingAddress.postalCode}</p><p>{selectedOrder.shippingAddress.country}</p>{selectedOrder.shippingAddress.mobile && (<p className="pt-2 text-xs">Phone: <a href={`tel:${selectedOrder.shippingAddress.mobile}`} className="text-blue-600 font-bold">{selectedOrder.shippingAddress.mobile}</a></p>)}<p className="text-xs">Email: <a href={`mailto:${selectedOrder.user?.email}`} className="text-blue-600">{selectedOrder.user?.email}</a></p></div>) : <p className="text-sm text-gray-500 italic">No shipping info available</p>}</div>
