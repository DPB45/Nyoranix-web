        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl p-8 animate-fade-in max-h-[95vh] overflow-y-auto">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              ...
              <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">5. Short Description</label><textarea name="shortDescription" value={formData.shortDescription} onChange={handleChange} rows="3" required className="w-full border rounded-lg p-3 text-sm"></textarea></div>
      ...
      {showOrderModal && selectedOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl p-8 animate-fade-in max-h-[90vh] overflow-y-auto">
            ...
            <div><h3 className="font-bold text-gray-700 mb-3 uppercase text-xs">Items to Pack ({selectedOrder.orderItems?.length || 0})</h3><div className="overflow-hidden border border-gray-200 rounded-lg"><table className="w-full text-sm text-left"><thead className="bg-gray-50 text-gray-500 font-medium"><tr><th className="px-4 py-2">Image</th><th className="px-4 py-2">Product Name</th><th className="px-4 py-2 text-center">Qty</th><th className="px-4 py-2 text-right">Price</th></tr></thead><tbody className="divide-y divide-gray-100">{selectedOrder.orderItems && selectedOrder.orderItems.map((item, index) => (<tr key={index}><td className="px-4 py-2"><img src={item.image} alt={item.name} className="w-10 h-10 object-contain border rounded bg-white" /></td><td className="px-4 py-2 font-medium text-gray-800">{item.name}</td><td className="px-4 py-2 text-center font-bold">{item.qty}</td><td className="px-4 py-2 text-right">₹{item.price * item.qty}</td></tr>))}</tbody></table></div></div>
